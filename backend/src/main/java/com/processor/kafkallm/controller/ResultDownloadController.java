...
/**
 * Controller REST para download dos resultados em Markdown.
 */
@Slf4j
@RestController
...
public class ResultDownloadController {
    ...
    /**
     * Endpoint para download do resultado em Markdown.
     */
    @GetMapping("/results/{fileId}/download")
    public ResponseEntity<Resource> downloadResult(@PathVariable String fileId) {
        log.info("Requisicao de download do resultado recebida para fileId: {}", fileId);

        try {
            Path filePath = storageService.getHtmlFilePath(fileId)
